...
const invoiceDraft = ref<{ lines: { sourceMovementId: string; concept: string; quantity: number; unitPriceCents: number; taxRate: number }[] } | null>(null);
const itemId = ref("");
...
async function load(): Promise<void> {
  ...
  const [movementResult, draftResult] = await Promise.allSettled([getMovementsForRepair(), getInvoiceDraft()]);
  movements.value = movementResult.status === "fulfilled" ? movementResult.value : [];
  invoiceDraft.value = draftResult.status === "fulfilled" ? draftResult.value : null;
}

async function getInvoiceDraft() {
  const response = await fetch(`/api/payments/draft/repair/${route.params.id}`, { headers: { authorization: `Bearer ${staffSession.value?.accessToken ?? ""}` } });
  if (!response.ok) return null;
  return response.json() as Promise<{ lines: { sourceMovementId: string; concept: string; quantity: number; unitPriceCents: number; taxRate: number }[] }>;
}
